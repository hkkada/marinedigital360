...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md font-medium transition-all disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
  {
    variants: {
      ...
      /**
        * Size owns font-size as well as height/padding. It used to be
        * `text-sm` in the base string above, so `size="lg"` grew the box but
        * left the label at 14px — which is why the hero's primary CTA read
        * smaller than every other page's.
        */
      size: {
        default: "h-9 px-4 py-2 has-[>svg]:px-3 text-meta",
        sm: "h-8 rounded-md gap-1.5 px-3 has-[>svg]:px-2.5 text-meta",
        lg: "h-10 rounded-md px-6 has-[>svg]:px-4 text-body",
        icon: "size-9 rounded-md text-meta",
      },
    },
    defaultVariants: {
      variant: "default",
